...
class Config():
    ...
    def load_model(self):
        ...
        # —————————————— # 导入共享参数
        checkpoint = torch.load(f'{self.DataPath}/.pt3/switchtransfomer.Transfomer_{self.d_model}_{self.SrcName}_{self.TgtName}_{self.num}.pt', map_location="cpu")
        ...
        for i in range(self.ExpertsFileNum):
            # 导入专家参数
            checkpoint = torch.load(f'{self.DataPath}/.pt3/switchtransfomer.ExpertsSet{i+1:>03}-of-{self.ExpertsFileNum:>03}_{self.d_model}_{self.SrcName}_{self.TgtName}_{self.num}.pt', map_location="cpu")
