...
import TestSwitchTransformer
import ExperimentSwitchTransformer
import TrainSwitchTransformer
import argparse
...
if __name__ == '__main__':
    ...
    group.add_argument("--DataPath"  , default='C:/Users/Lenovo/Desktop/jy/Data/'  , type=str  , help="数据库地址")
    ...
    TrainSwitchTransformer.add_subparser(subparsers, parents=[parent_parser])
    TestSwitchTransformer.add_subparser(subparsers, parents=[parent_parser])
    ExperimentSwitchTransformer.add_subparser(subparsers, parents=[parent_parser])
    
    # —————————————— # 处理接收的参数
    #args = parser.parse_args()
    args = parser.parse_args(['experiment','--Greedy','True','--Beam','False','--num','100000','--GpuNum','1','--batch_size','8'])
    assert args.ExpertNum % args.GpuNum == 0, "ExpertNum 必须能被 GpuNum 整除"
    args.func(args)
    
# .\\venv\\Scripts\\python.exe .\\SwitchTransformer\\main.py ppc --DP True --train True --use True # 数据预处理
# screen -dmS test bash -c "./venv/bin/torchrun --nproc-per-node 2 ./SwitchTransformer-3/main.py train --DataPath /root/autodl-tmp/Data/ --GpuNum 2" # 训练 RTX 5090 * 2