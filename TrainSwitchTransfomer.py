...
def make_named(switchtransfomer):
    switchtransfomer_named = list(switchtransfomer.transfomer.named_parameters())
    shared_named = [(n, p) for (n, p) in switchtransfomer_named if "router" not in n]
    router_named = [(n, p) for (n, p) in switchtransfomer_named if "router" in n]
    expert_named = list(switchtransfomer.expertset.named_parameters())
    return shared_named,router_named,expert_named

def save_model(args,switchtransfomer,ProcessId,steps):
    torch.save(switchtransfomer.expertset.state_dict(),f'{args.DataPath}/.pt/switchtransfomer.ExpertsSet{ProcessId+1:>03}-of-{args.GpuNum:>03}_{args.d_model}_{args.SrcName}_{args.TgtName}_{str(steps)}.pt')
    if ProcessId == 0:
...
def function(args: argparse.Namespace):
    ...
    switchtransfomer = modelconfig.make_model()

    # —————————————— # 配置参数优化器
    shared_named,router_named,expert_named = make_named(switchtransfomer)
        ...
        while True:
            for batch in loader:
                ...
                with autocast(device_type='cuda',dtype=torch.bfloat16):
                    out,router_loss = switchtransfomer(src,tgt[:, :-1])
                ...
                # —————————————— # 保存模型
                if steps%10000==0:
                    save_model(args,switchtransfomer,ProcessId,steps)
...
def add_subparser(subparsers: argparse._SubParsersAction, parents=None):
    ...
    group.add_argument("--z_coef" , default=0.01 , type=str2float, help="z_loss系数")
