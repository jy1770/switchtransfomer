...
from experiment.ExpertSet_ import *
from experiment.Transformer_  import *
from experiment.SwitchTransformer_  import *
...
class Config():
    ...
    def load_model(self):
        ...
        transformer = Transformer(encoder,decoder,self.PadId,self.h,self.device,self.Occlusion_ModelName,self.Occlusion_NIdx,self.Occlusion_ExpertIdx)
        expertsset = ExpertsSet(self.d_model,self.d_ff,self.ExpertNum,devices,self.N)

        # —————————————— # 导入共享参数
        checkpoint = torch.load(f'{self.DataPath}/.pt1/switchtransformer.transformer_{self.d_model}_{self.SrcName}_{self.TgtName}_{self.num}.pt', map_location="cpu")
        transformer.load_state_dict(checkpoint, strict=True)
        ...
        for i in range(self.ExpertsFileNum):
            # 导入专家参数
            checkpoint = torch.load(f'{self.DataPath}/.pt1/switchtransformer.expertset{i+1:>03}-of-{self.ExpertsFileNum:>03}_{self.d_model}_{self.SrcName}_{self.TgtName}_{self.num}.pt', map_location="cpu")
            for expert_idx in range(self.ExpertNum // self.ExpertsFileNum):
                ...
                idx += 1
        expertsset.load_state_dict(expertsset_state_dict, strict=True)
        ...
        # —————————————— # 组合模型
        switchtransformer = SwitchTransformer(transformer,expertsset)
