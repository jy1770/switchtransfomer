...
class Switch(nn.Module):
    def __init__(self,d_model,ExpertNum,ModelName,NIdx):
        super().__init__()
        self.ModelName = ModelName
        self.NIdx = NIdx
        self.ExpertNum = ExpertNum
        self.device='cuda:0'
        self.router  = NetWorkRouter(d_model,ExpertNum)

    def forward(self,x,experts,token_mask,Occlusion_ModelName,Occlusion_NIdx,Occlusion_ExpertIdx):
        # —————————————— # 参数初始化
        B, T, D = x.shape
        ...
        if self.ModelName==Occlusion_ModelName and self.NIdx==Occlusion_NIdx:
            logits[:,Occlusion_ExpertIdx] = -float("inf")
