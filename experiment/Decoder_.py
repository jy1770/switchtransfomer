...
from experiment.PositionalEncoding_ import*
from experiment.Switch_ import *

class DecoderLayer(nn.Module):
    def __init__(self,d_model,h,ExpertNum,NIdx):
        ...
        self.Switch = Switch(d_model,ExpertNum,'tgt',NIdx)
    def forward(self,tgt,src,tgt_mask,tgt_pad_mask,src_pad_mask,token_mask,experts,Occlusion_ModelName,Occlusion_NIdx,Occlusion_ExpertIdx):
        # —————————————— # 自注意力
        ...
        tgt_ = self.Switch(tgt_, experts,token_mask,Occlusion_ModelName,Occlusion_NIdx,Occlusion_ExpertIdx)
        tgt += tgt_
        return tgt
...
class Decoder(nn.Module):
    def __init__(self,d_model,h,N,vocab_size,ExpertNum):
        ...
        self.layers  = nn.ModuleList([DecoderLayer(d_model,h,ExpertNum,n) for n in range(N)])
        self.fc_out  = nn.Linear(d_model, vocab_size)
        self.EndLayerNorm = nn_LayerNorm(d_model)
        # —————————————— # 初始化参数
        self.N = N
        self.scale = d_model**0.5
    def forward(self,tgt,src,tgt_mask,tgt_pad_mask,src_pad_mask,expertsset,Occlusion_ModelName,Occlusion_NIdx,Occlusion_ExpertIdx):
        # —————————————— # 嵌入
        ...
        for layer,experts in zip(self.layers,expertsset):
            tgt = layer(tgt,src,tgt_mask,tgt_pad_mask,src_pad_mask,token_mask,experts,Occlusion_ModelName,Occlusion_NIdx,Occlusion_ExpertIdx)
        # —————————————— # 合成输出
