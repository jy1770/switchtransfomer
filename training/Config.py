...
class Config():
    ...
    def make_model(self):
        # —————————————— # 实例化模型
        encoder = Encoder(self.d_model,self.h,self.N,self.vocab_size,self.ExpertNum,self.local_ExpertNum,self.GpuNum,self.dropout,self.ProcessId,self.capacity_factor,self.sigma,self.group,self.lb_coef,self.z_coef)
        decoder = Decoder(self.d_model,self.h,self.N,self.vocab_size,self.ExpertNum,self.local_ExpertNum,self.GpuNum,self.dropout,self.ProcessId,self.capacity_factor,self.sigma,self.group,self.lb_coef,self.z_coef)
        transfomer = Transfomer(encoder, decoder, self.PadId, self.h, self.N).cuda()
        ...
        # —————————————— # DDP包装
        transfomer = DDP(transfomer,process_group=self.group)
