...
def cal_z_loss(logits_all,N_all):
    z_kept = torch.logsumexp(logits_all, dim=-1)    # 只算 kept 的
    z_sum = (z_kept ** 2).sum()
    z_loss = z_sum / N_all
    return z_loss
...
class NetWorkRouter(nn.Module):
    def __init__(self,d_model,ExpertNum,sigma,lb_coef,z_coef):
        super().__init__()
        # —————————————— # 模型参数
    ...
    def forward(self,x_all,N_all):
        ...
        z_loss = cal_z_loss(logits_all,N_all)
        loss = self.z_coef*z_loss + self.lb_coef*lb_loss
        ...
        del logits_all,lb_loss,z_loss
        return probs_all,expert_idx_all,loss

class Switch(nn.Module):
    def __init__(self,d_model,ExpertNum,local_ExpertNum,GpuNum,ProcessId,sigma,group,lb_coef,z_coef):
        super().__init__()
        self.ExpertNum = ExpertNum
        ...
        self.group = group
        self.router = NetWorkRouter(d_model,ExpertNum,sigma,lb_coef,z_coef)
