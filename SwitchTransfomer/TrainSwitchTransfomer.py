...
def function(args: argparse.Namespace):
    ...
    if ProcessId == 0:
        TrainLossPath = f'{args.DataPath}/TrainLoss/TrainLoss_11.txt'
    ...
    # —————————————— # 实例并应用模型参数配置器(Config)
    switchtransfomerconfig = Config(args,PadId,device,ProcessId,group)
    switchtransfomer = switchtransfomerconfig.make_model()
    # —————————————— # 配置参数优化器
    # —————— # 【配置一 : transfomer配置】
    '''
    optimizer = optim.Adam(switchtransfomer.parameters(),lr=1.0,betas=(0.9, 0.98),eps=1e-9)
    ...
    '''
    # —————— # 【配置二 : 不同的功能使用不同的优化策略】
    shared_named,router_named,experts_named = make_named(switchtransfomer)
        ...
        while True:
            for batch in loader:
                ...
                with autocast(device_type='cuda',dtype=torch.bfloat16):
                    out,lb_loss ,z_loss = switchtransfomer(src, tgt[:, :-1])
                ...
                shared_params = [p for p in switchtransfomer.Transfomer.parameters() if p.grad is not None]
                torch.nn.utils.clip_grad_norm_(shared_params, args.clip)
                # 专家参数单独裁剪（可选，但建议给一个更大的阈值或单独开关）
                expert_params = [p for p in switchtransfomer.ExpertsSet.parameters() if p.grad is not None]
...
def add_subparser(subparsers: argparse._SubParsersAction, parents=None):
    ...
    group.add_argument("--clip"   , default=1     , type=str2int  , help="梯度剪切")
