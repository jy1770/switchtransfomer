...
class DecoderLayer(nn.Module):
    ...
    def forward(self,tgt,src,tgt_mask,tgt_pad_mask,src_pad_mask,experts):
        # —————————————— # 自注意力
...
class Decoder(nn.Module):
    ...
    def forward(self,tgt,src,tgt_mask,tgt_pad_mask,src_pad_mask,expertsset):
        ...
        for i,layer in enumerate(self.layers):
            tgt,lb_loss,z_loss = layer(tgt,src,tgt_mask,tgt_pad_mask,src_pad_mask,expertsset[i])
            lb_loss_list.append(lb_loss) ; z_loss_list.append(z_loss)
        # —————————————— # 合成输出
        tgt = self.EndLayerNorm(tgt)
        output = self.fc_out(tgt)
        return output,torch.stack(lb_loss_list).mean(),torch.stack(z_loss_list).mean()