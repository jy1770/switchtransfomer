...
class Switch(nn.Module):
    ...
    def forward(self,x,experts,token_mask):
        ...
        # —————————————— # 获取被筛选后的token数量 N(全局)
        num_all_local = torch.tensor([N_all], device=self.device, dtype=torch.int64)
        dist.all_reduce(num_all_local, op=dist.ReduceOp.SUM, group=self.group)
        num_all_global = int(num_all_local.item())
        '''
        num_kept_local = torch.tensor([N_send], device=self.device, dtype=torch.int64)
        ...
        # —————————————— # 第一次 all-to-all
        recv_x = torch.empty((total_recv, D), device=self.device, dtype=x.dtype)
        recv_gate = torch.empty((total_recv,), device=self.device, dtype=x.dtype)
        recv_local_eid = torch.empty((total_recv,), device=self.device, dtype=torch.int64)
        ...
        # —————————————— # 本地expert计算
        y_recv = torch.zeros((total_recv, D), device=self.device, dtype=x.dtype)
        y_recv = experts(y_recv, recv_x, recv_gate, recv_local_eid)

        # —————————————— # 第二次 all-to-all
        y_perm = torch.empty((N_send, D), device=self.device, dtype=x.dtype)
        distnn.all_to_all_single(y_perm, y_recv,output_split_sizes=send_splits,input_split_sizes=recv_splits,group=self.group)

        # —————————————— # 反排序回 idx_send 对应的顺序
        y_sel = torch.empty((N_send, D), device=self.device, dtype=x.dtype)
        y_sel.index_copy_(0, perm, y_perm)

        # —————————————— # 复原参数
        y_flat_all = torch.zeros((tokens_per_core, D), device=self.device, dtype=x.dtype)
