...
def Beam(args: argparse.Namespace):
    ...
    with torch.inference_mode():
        for st in tqdm(range(0, len(src_ids_list), args.batch_size),
                       desc=f'Beam评估(beam=4,alpha=0.6): transfomer_{args.d_model}_{args.SrcName}_{args.TgtName}_{args.num}.pt'):
            batch_ids = src_ids_list[st: st + args.batch_size]
            B = len(batch_ids)
            max_src = max(len(x) for x in batch_ids)
