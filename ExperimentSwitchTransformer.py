...
from experiment.Config_ import *
...
def Greedy(args: argparse.Namespace):
    ...
    modelconfig = Config(args, PadId, device)
    model = modelconfig.load_model()
    ...
    with torch.inference_mode(): # inference_mode 比 no_grad 更快/更省
        for st in tqdm(range(0, len(src_ids_list), args.batch_size),desc=f'ModelName:{args.Occlusion_ModelName}  Nidx:{args.Occlusion_NIdx}  ExpertIdx:{args.Occlusion_ExpertIdx}'):
            ...
            # ——— Encoder 一次跑完
            src, src_pad_mask = model.transformer.forward_Encoder(src, model.expertset)
            ...
            for _ in range(args.max_len):
                out = model.transformer.forward_Decoder(tgt, src, src_pad_mask, model.expertset)  # (B, t, V)
...
def beam_search_batch(model,src_enc,src_pad_mask,bos_id,eos_id,pad_id,max_len=256,beam_size=4,alpha=0.6):
    """
    ...
    for step in range(1, max_len + 1):
        ...
        out = model.Transformer.forward_Decoder(tgt_flat, src_enc_beam, src_pad_beam, model.ExpertsSet)
...
def Beam(args: argparse.Namespace):
    ...
    modelconfig = Config(args, PadId, device)
    model = modelconfig.load_model().eval()

    # —————————————— # 获取参考集（AIAIN 通常是 case-sensitive，因此不要 lowercase）
    src_lines = read_sgm_segs(SrcFilePath, lowercase=False)
    ref_texts = read_sgm_segs(RefFilePath, lowercase=False)
    print(f'SrcFilePath : {SrcFilePath}')
    print(f'RefFilePath : {RefFilePath}')
    ...
    with torch.inference_mode():
        for st in tqdm(range(0, len(src_ids_list), args.batch_size),desc=f'Beam评估(beam=4,alpha=0.6): switchtransformer_{args.d_model}_{args.SrcName}_{args.TgtName}_{args.num}.pt'):
            batch_ids = src_ids_list[st: st + args.batch_size]
            B = len(batch_ids)
            max_src = max(len(x) for x in batch_ids)
            ...
            # Encoder
            src_enc, src_pad_mask = model.transformer.forward_Encoder(src, model.expertset)

            # Beam search decode
            batch_out_ids = beam_search_batch(model,src_enc,src_pad_mask,BosId, EosId, PadId,max_len=args.max_len,beam_size=beam_size,alpha=alpha)
...
def add_subparser(subparsers: argparse._SubParsersAction, parents=None):
    if parents is None:
        parents = []
    parser = subparsers.add_parser('experiment', help='数据处理',parents=parents)
    ...
    # —————————————— # 评估专家参数
    group.add_argument("--Occlusion_ModelName"   , default='src', type=str    , help="屏蔽的模块")
    group.add_argument("--Occlusion_NIdx"     , default=0    , type=str2int, help="屏蔽的层数")
    group.add_argument("--Occlusion_ExpertIdx", default=0    , type=str2int, help="Greedy评估batch大小")
