...
def Greedy(args: argparse.Namespace):
    ...
    with torch.inference_mode(): # inference_mode 比 no_grad 更快/更省
        for st in tqdm(range(0, len(src_ids_list), args.batch_size),desc=f'正在评估:switchtransformer_{args.d_model}_{args.SrcName}_{args.TgtName}_{args.num}.pt'):
            ...
            # ——— Encoder 一次跑完
            src, src_pad_mask = model.Transformer.forward_Encoder(src, model.ExpertsSet)
            ...
            for _ in range(args.max_len):
                out = model.Transformer.forward_Decoder(tgt, src, src_pad_mask, model.ExpertsSet)  # (B, t, V)
...
def Beam(args: argparse.Namespace):
    ...
    with torch.inference_mode():
        for st in tqdm(range(0, len(src_ids_list), args.batch_size),desc=f'Beam评估(beam=4,alpha=0.6): switchtransformer_{args.d_model}_{args.SrcName}_{args.TgtName}_{args.num}.pt'):
            ...
            # Encoder
            src_enc, src_pad_mask = model.Transformer.forward_Encoder(src, model.ExpertsSet)
